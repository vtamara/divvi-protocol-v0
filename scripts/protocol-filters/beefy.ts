...
export async function filterEvents(events: ReferralEvent[]): Promise<ReferralEvent[]> {
...
async function filter(event: ReferralEvent): Promise<boolean> {
    ...
    return transactions.every(transaction => new Date(transaction.datetime).getTime() > event.timestamp) && transactions.length > 0;
}
...
async function fetchWithBackoff(url: string, options: RequestInit, retries: number = 5, delay: number = 1000): Promise<Response> {
    try {
...
async function fetchInvestorTimeline(address: string): Promise<BeefyInvestorTransaction[]> {
