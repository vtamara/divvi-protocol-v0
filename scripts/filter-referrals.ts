...
async function main() {
    ...
    const referralEvents: ReferralEvent[] = readFileSync(args.input, 'utf8').split('\n').map((line) => {
    ...
    const output = filteredEvents.map((event) => `${event.userAddress},${event.timestamp}`).join('\n')
